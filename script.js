...
(function() {
  var Monomial, Polynomial, Term, gcd, getValue, gridsUnder, grobnerBasis, notFound, println, quotientBasis, reduce, sPolynomial, setDisabled, setValue, sortBy, variableNames, variables,
    slice = [].slice;
  ...
  grobnerBasis = function(fs) {
    var f, gs, i, j, k, l, len, n, ref, ref1, ref2, s;
    fs = fs.slice();
    while (true) {
      println("in grobnerBasis, fs: " + fs);
      gs = [];
      for (i = k = 0, ref = fs.length; 0 <= ref ? k < ref : k > ref; i = 0 <= ref ? ++k : --k) {
        for (j = l = ref1 = i + 1, ref2 = fs.length; ref1 <= ref2 ? l < ref2 : l > ref2; j = ref1 <= ref2 ? ++l : --l) {
          s = sPolynomial(fs[i], fs[j]);
          for (n = 0, len = fs.length; n < len; n++) {
            f = fs[n];
            s = reduce(s, f);
          }
          if (!s.isZero()) {
  ...
  reduce = function(f1, f2) {
    ...
    while (true) {
      ...
      l1 = f1.leading();
      g = Monomial.gcd(l1, l2);
      if (!g.term.equals(l2.term)) {
        return f1;
      }
      f1 = f1.times(Polynomial.from(l2.divide(g))).subtract(f2.times(Polynomial.from(l1.divide(g))));
    }
  };

  quotientBasis = function(generators) {
    var generator, k, l, len, len1, limit, limits, terms;
    limits = variables.map(function(x) {
      var generator, k, len, limit;
      limit = notFound;
    ...
    for (k = 0, len = limits.length; k < len; k++) {
      limit = limits[k];
      if (limit === notFound) {
        return notFound;
      }
    ...
    for (l = 0, len1 = generators.length; l < len1; l++) {
  ...
  Monomial = (function() {
    ...
    Monomial.parse = function(str) {
      var coefficient, cstr, i, indices, istr, k, len, ref, ref1, x, xistr, xistrs, xstr;
